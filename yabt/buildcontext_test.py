...
def test_continue_after_fail(basic_conf):
    ...
    on A and C respectively. A is a faulty build that will cause C to be skipped.
    D will pass after B is built.
    To test that we "continued after fail" we need to make sure A runs and fails
    before we reach B.
    This is what we want:
    A -X-> C (skipped)
    B --> D
    Such test is icky, because we can't promise anything about the order in which
    indepedant branches run.
    If BD would run before AC we can't distinguish that behavior from AC failing
    and BD running thanks to the flag.
    Sope, we generate 10 AC-alikes and 10 BD-alikes, and statistically assume at
    least one BD pair would run after an AD pair.
